...
class Client:
    ...
    def start_client(self):
        ...
        text_name = font.render('Возможные имена: Danya, Leha, Sanya, Seva, Vanya, Vova', True, THECOLORS['black'])
        place_text_name = text_name.get_rect(center=(750, 25))
        ...
        while tmp:
            for event in pygame.event.get():
                # Теперь человек выбирает букву и мы ее отрисуем
                if event.type == pygame.KEYDOWN:
                    ...
                    if number_letter == 0:
                        number_letter += 1
                        name_gamer += tmp_letter.upper()
                    elif tmp_letter == 'stop':
                        tmp = False
                        break
                    else:
                        name_gamer += tmp_letter
                    pygame.draw.polygon(dis, THECOLORS['lightskyblue3'],
                                        [[500, 300], [1000, 300], [1000, 600], [500, 600]])
                    text_name = font.render(name_gamer[5:], True, THECOLORS['black'])
                    place_text_name = text_name.get_rect(center=(750, 500))
                    dis.blit(text_name, place_text_name)
                    pygame.display.update()
        ...
        text_name = font.render(f"Я игрок {name_gamer[5:]}", True, THECOLORS['black'])
        ...
        # sock.connect(('gra6er.ru', 10000))
        sock.connect(('localhost', 10000))
        sock.send(name_gamer.encode())
